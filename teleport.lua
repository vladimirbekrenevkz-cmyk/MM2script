...
local function updateESP()
    -- Очищаем только старые хайлайты
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            ...
            -- Если хайлайт уже есть, обновляем цвет
...
local function createGui()
    if PlayerGui:FindFirstChild("Def1x") then return end
        ...
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if targetHRP.Position.Y > 10 then
                    ...
                    task.wait(0.15)
                    if knife and knife.Parent == char and knife:FindFirstChild("Handle") and knife.Activate then
                        knife:Activate()
                    end
                end
            end
...
-- Обновление ESP каждые 0.5 секунды (оптимизация)
spawn(function()
    while wait(0.5) do
        if espEnabled then
            updateESP()
